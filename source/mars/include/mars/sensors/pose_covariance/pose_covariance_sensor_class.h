...
#ifndef POSECOVARIANCESENSORCLASS_H
#define POSECOVARIANCESENSORCLASS_H

#include <mars/core_state.h>
#include <mars/ekf.h>
#include <mars/general_functions/utils.h>
#include <mars/sensors/bind_sensor_data.h>
#include <mars/sensors/pose_covariance/pose_covariance_measurement_type.h>
#include <mars/sensors/pose_covariance/pose_covariance_sensor_state_type.h>
...
namespace mars
{
using PoseCovarianceSensorData = BindSensorData<PoseCovarianceSensorStateType>;

class PoseCovarianceSensorClass : public UpdateSensorAbsClass
{
public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW

  PoseCovarianceSensorClass(std::string name, std::shared_ptr<CoreState> core_states)
  {
    name_ = std::move(name);
  ...
  virtual ~PoseCovarianceSensorClass() = default;

  PoseCovarianceSensorStateType get_state(std::shared_ptr<void> sensor_data)
  {
    PoseCovarianceSensorData data = *static_cast<PoseCovarianceSensorData*>(sensor_data.get());
    return data.state_;
  }
  ...
  {
    PoseCovarianceMeasurementType measurement = *static_cast<PoseCovarianceMeasurementType*>(sensor_data.get());

    PoseCovarianceSensorData sensor_state;
    ...
    if (this->initial_calib_provided_)
    {
      ...
      PoseCovarianceSensorData calib = *static_cast<PoseCovarianceSensorData*>(initial_calib_.get());
    ...
    BufferDataType result(std::make_shared<CoreType>(*latest_core_data.get()),
                          std::make_shared<PoseCovarianceSensorData>(sensor_state));
                  ...
                  BufferDataType* new_state_data)
  {
    ...
    PoseCovarianceMeasurementType* meas = static_cast<PoseCovarianceMeasurementType*>(measurement.get());
    PoseCovarianceSensorData* prior_sensor_data = static_cast<PoseCovarianceSensorData*>(latest_sensor_data.get());
    ...
    const PoseCovarianceSensorStateType corrected_sensor_state = ApplyCorrection(prior_sensor_state, sensor_correction);
    ...
    // SensorState data
    std::shared_ptr<PoseCovarianceSensorData> sensor_data(std::make_shared<PoseCovarianceSensorData>());
  ...
  PoseCovarianceSensorStateType ApplyCorrection(const PoseCovarianceSensorStateType& prior_sensor_state, const Eigen::MatrixXd& correction)
  {
    // state + error state correction
    // with quaternion from small angle approx -> new state

    PoseCovarianceSensorStateType corrected_sensor_state;
...
#endif  // POSECOVARIANCESENSORCLASS_H
